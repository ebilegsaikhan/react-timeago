...
var TimeAgo = function (_Component) {
  ...
  _createClass(TimeAgo, [{
    ...
    value: function render() {
      ...
      var now = Date.now();
      var seconds = Math.round(Math.abs(now - then) / 1000);
      var suffix = then < now ? 'өмнө' : 'дараа';

      var _ref = seconds < MINUTE ? [Math.round(seconds), 'секундын'] : seconds < HOUR ? [Math.round(seconds / MINUTE), 'минутын'] : seconds < DAY ? [Math.round(seconds / HOUR), 'цагын'] : seconds < WEEK ? [Math.round(seconds / DAY), 'өдрийн'] : seconds < MONTH ? [Math.round(seconds / WEEK), 'долоо хоногийн'] : seconds < YEAR ? [Math.round(seconds / MONTH), 'сарын'] : [Math.round(seconds / YEAR), 'жилийн'];
